...
import { registerAccountSchema } from '@/lib/validators/auth';

const INVALID_OR_EXPIRED = 'Reset code not found or has expired. Request a new one.';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const passwordCheck = registerAccountSchema.shape.password.safeParse(password);
    if (!passwordCheck.success) {
      return NextResponse.json({ error: passwordCheck.error.issues[0].message }, { status: 400 });
    }
